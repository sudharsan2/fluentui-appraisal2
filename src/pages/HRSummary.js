...
const HRReviewer = (props) => {
 ...
 const handleCheckboxChange = (event, empId) => {
  ...
  const fetchEmployeeData = () => {
    axios.get('http://127.0.0.1:8004/user/hrsummarylist')
      .then(response => {
  ...
  const onActiveOptionChange3 = React.useCallback(
    async (event, data) => {
      ...
      try {
        const response1 = await axios.get(`http://127.0.0.1:8004/user/team-member/remarks/${selectedEmployee.employee_id}/${data?.nextOption?.text}`);
      ...
      try {
        const response2 = await axios.get(`http://127.0.0.1:8004/user/appraiser/remarks/${selectedEmployee.employee_id}/${data?.nextOption?.text}`);
      ...
      try {
        const response3 = await axios.get(`http://127.0.0.1:8004/user/reviewer/remarks/${selectedEmployee.employee_id}/${data?.nextOption?.text}`);
