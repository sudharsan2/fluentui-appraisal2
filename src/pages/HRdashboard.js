...
const mdata = {
    ...
    'Manager 2': [
      ...
      { name: 'Yet to be Filled', value: 10 },
    ...
    // Add more data for other managers as needed
  };
...
const HRDashboard = () => {
  ...
  const [selectedDate, setSelectedDate] = useState(null);
  const [selectedManager, setSelectedManager] = useState('Manager 1');
  const [managerData, setManagerData] = useState(mdata[selectedManager]);
 
  const [selectedReviewer, setSelectedReviewer] = useState('Reviewer 1');
  const [reviewerData, setReviewerData] = useState(rdata[selectedReviewer]);
...
//   ]
  ...
  useEffect(() => {
    axios.get('http://172.232.97.231:5051/user/fullcalender')
      .then(response => {
...
//   }, { Employee: 5, Manager: 2, Reviewer: 7 });
  ...
  const handleReviewerChange = (event, item) => {
    if (item) {
      setSelectedReviewer(item.key);
      setReviewerData(rdata[item.key]);
    }
  };
...
<div className="widget widget-pie">
      ...
      <Dropdown
        ...
        onChange={handleManagerChange}
        styles={{ dropdown: { width: '30%', marginBottom: 20 } }}
      />
    ...
    <div className="widget bar">
      ...
      />
      <div style={{display:"flex", justifyContent:"center"}}>
      <BarChart width={400} height={250} data={reviewerData} align="center">
      {/* <CartesianGrid strokeDasharray="3 3" /> */}
      <XAxis dataKey="name" />
      <YAxis/>
        <RechartsTooltip
